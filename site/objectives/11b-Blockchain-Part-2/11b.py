...
class Block():
        ...
        if(genesis == True):
            ...
            if(load == False):
                if all(p is not None for p in [index, block_data['documents'], block_data['pid'], block_data['rid'], block_data['score'], block_data['sign'], previous_hash]):
                    ...
                    if self.sha256er() == '58a3b9335a6ceb0234c12d35a0564c4ef0e90152d0eb2ce2082383b38028a90f': #MLR
                        print(self) #MLR
                else:
                    return None
    ...
    def block_data(self):
        ...
        #print(s)
        #sha256er = SHA256.new(s).hexdigest() #MLR
        #print ("Added block data to %s sha256: %s" % (str(self.index), sha256er)) #MLR
        return(s)
    ...
    def load_a_block(self, fh):
        ...
        if self.nonce in MLR:
            MLR[self.nonce] = MLR[self.nonce].append(self.index)
        else:
            MLR[self.nonce] = [self.index]
        frosty = fh.read(16)
        frostyblock += frosty
        ...
        #newhash, self.sig= self.hash_n_sign() #MLR
        #print(self.hash, newhash) #MLR
        #print(self.sha256er()) #MLR
        frosty = fh.read(344)
        frostyblock += frosty
        self.sig = frosty
        #print(self.hash, self.sig) #MLR
        #self.hash, self.sig = self.hash_n_sign() #MLR
        #print(self.hash, self.sig) #MLR
        #if self.index>=128447 and self.index<=128450: #MLR
        #    print(self) #MLR
    ...
    def dump_doc(self, doc_no):
        ...
        with open(filename, 'wb') as fh:
            hash_obj = SHA256.new(self.data[doc_no - 1]['data']).hexdigest() #MLR
            print('Doc hash : %s' % (hash_obj)) #MLR
...
class Chain():
    ...
    def add_block(self, block_data):
        ...
        print ("Added block %s sha256: %s" % (str(self.blocks[i].index), b.sha256er())) #MLR
    ...
    def verify_chain(self, publickey, previous_hash=None):
        ...
        for i in range(0, len(self.blocks)):  # assume Genesis block integrity
            ...
            sha256er = SHA256.new(self.blocks[i].block_data()).hexdigest() #MLR
            print ("Block %s sha256: %s" % (str(self.blocks[i].index), sha256er)) #MLR
...
#
    ...
    #c2 = Chain(load=True, filename='m1.pdf')
    ...
    for k in MLR.keys():
        if len(MLR[k])>1:
            print (MLR[k])
    #for item in range(0,1548):
    #    c2.blocks[item].dump_doc(1)
